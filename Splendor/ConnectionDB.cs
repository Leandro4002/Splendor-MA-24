...
namespace Splendor
{
    ...
    class ConnectionDB
    {
        ...
        {
            string sql = "CREATE DATABASE splendorDB";
        ...
        private void CreateInsertPlayer()
        {
            string sql = "CREATE TABLE player (id INT PRIMARY KEY, pseudo VARCHAR(20));" +
                "insert into player (id, pseudo) values (0, 'Fred');" +
                "insert into player (id, pseudo) values (1, 'Harry');" +
                "insert into player (id, pseudo) values (2, 'Sam');";
            SQLiteCommand command = new SQLiteCommand(sql, m_dbConnection);
            command.ExecuteNonQuery();
        }
        ...
        public Stack<Card> GetCardAccordingToLevel(int level)
        {
